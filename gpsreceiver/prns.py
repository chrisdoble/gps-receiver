...
# The C/A PRNS of all GPS satellites, indexed by satellite ID.
PRNS_BY_SATELLITE_ID: dict[SatelliteId, np.ndarray] = {}

for satellite_id, outputs in _prn_outputs.items():
    g1 = _lfsr([10], [3, 10])
    g2 = _lfsr(outputs, [2, 3, 6, 8, 9, 10])
    prn = np.empty(1023, np.float32)
    for i in range(1023):
        prn[i] = (next(g1) + next(g2)) % 2
    PRNS_BY_SATELLITE_ID[satellite_id] = prn

# The same C/A PRNs as above, but upsampled so the number of chips in each is
# equal to the number of samples present in 1 ms of a received signal.
#
...
assert (
    SAMPLES_PER_MILLISECOND % len(PRNS_BY_SATELLITE_ID[1]) == 0
), "SAMPLES_PER_MILLISECOND isn't an integer multiple of the number of chips in a C/A PRN code (1023)"
_repeat_count = SAMPLES_PER_MILLISECOND // len(PRNS_BY_SATELLITE_ID[1])
UPSAMPLED_PRNS_BY_SATELLITE_ID = {
    satellite_id: np.repeat(prn, int(_repeat_count))
    for satellite_id, prn in PRNS_BY_SATELLITE_ID.items()
}

# The same upsampled C/A PRNs as above, but with 0s replaced with -1.
#
...
COMPLEX_UPSAMPLED_PRNS_BY_SATELLITE_ID = {
    satellite_id: np.array([-1 if b == 0 else 1 for b in prn])
    for satellite_id, prn in UPSAMPLED_PRNS_BY_SATELLITE_ID.items()
}
